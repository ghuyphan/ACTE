...
function rowToFriend(row: FriendRow): FriendConnection {
  return {
    ...
    createdByInviteToken: row.created_by_invite_token,
  };
}
...
export async function getCachedSharedInvite(userUid: string): Promise<FriendInvite | null> {
  const db = await getDB();
