...
describe('CaptureCard doodle handle', () => {
  ...
  it('keeps the first sticker when adding a second sticker from the source sheet', async () => {
    const ref = React.createRef<CaptureCardHandle>();
    mockShouldImportSourceDirectlyAsSticker.mockResolvedValue(true);
    mockImagePicker.launchImageLibraryAsync
      .mockResolvedValueOnce({
        ...
        assets: [
          {
            uri: 'file:///transparent-sticker-1.png',
            mimeType: 'image/png',
            fileName: 'transparent-sticker-1.png',
          },
        ],
      })
      .mockResolvedValueOnce({
        ...
        assets: [
          {
            uri: 'file:///transparent-sticker-2.png',
            mimeType: 'image/png',
            fileName: 'transparent-sticker-2.png',
          },
        ],
      });
    ...
    await waitFor(() => {
      expect(ref.current?.getStickerSnapshot().placements).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ id: 'placement-1', zIndex: 1 }),
          expect.objectContaining({ id: 'placement-2', zIndex: 2 }),
        ])
      );
    });
  });
