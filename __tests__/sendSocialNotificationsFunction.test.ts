...
describe('send-social-notifications edge function', () => {
  ...
  it('fails closed when the idempotency claim RPC is unavailable', async () => {
    (globalThis as typeof globalThis & {
      allowConsoleMessagesForTest?: (
        patterns: Array<{ method: 'error'; pattern: RegExp }>
      ) => void;
    }).allowConsoleMessagesForTest?.([
      {
        method: 'error',
      ...
      {
        method: 'error',
        pattern: /^send-social-notifications failed: NotificationControlUnavailableError:/,
      },
    ]);
    const env = {
      SUPABASE_URL: 'https://example.supabase.co',
  ...
  it('fails closed when the recipient throttle RPC is unavailable', async () => {
    (globalThis as typeof globalThis & {
      allowConsoleMessagesForTest?: (
        patterns: Array<{ method: 'error'; pattern: RegExp }>
      ) => void;
    ...
    const rpc = jest.fn(async (functionName: string, args: Record<string, unknown>) => {
      ...
      if (functionName === 'reserve_social_notification_delivery') {
        return {
          ...
          error: {
            code: '42883',
            message: 'function reserve_social_notification_delivery does not exist',
          },
        };
      }

      if (functionName === 'release_social_notification_event') {
        return { data: null, error: null };
      }
    ...
    expect(fetchMock).not.toHaveBeenCalled();
    expect(rpc).toHaveBeenCalledWith(
      'release_social_notification_event',
      expect.objectContaining({
        actor_user_id_input: 'actor-1',
        event_type_input: 'shared_post_created',
        resource_id_input: 'post-1',
  ...
  it('does not fail the edge request when Expo rejects shared response delivery', async () => {
    const env = {
      SUPABASE_URL: 'https://example.supabase.co',
      SUPABASE_ANON_KEY: 'anon-key',
      SUPABASE_SERVICE_ROLE_KEY: 'service-role-key',
    ...
    const fetchMock = jest.fn(async (_input: string | URL | Request, _init?: RequestInit) =>
      Response.json({ errors: [{ message: 'invalid access token' }] }, { status: 401 })
    ) as jest.MockedFunction<typeof fetch>;
    const handler = loadSendSocialNotificationsHandler({
      createClient,
      env,
