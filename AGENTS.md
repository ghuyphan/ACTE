...
- `app.config.ts`: Expo config, native package IDs, Firebase service files, widgets, permissions, custom plugins, and env-backed native settings.
...
- Treat `app.config.ts`, `plugins/`, `ios/`, and `android/` as native-build-sensitive. Change them only when the task is explicitly about native config, widgets, or Firebase setup.
