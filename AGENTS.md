...
Noto is an Expo SDK 55 / React Native app that stores location-linked text notes and photo memories. It uses Expo Router for navigation, SQLite for local persistence, optional Supabase Auth + Postgres sync, geofencing reminders, and an Expo widget with custom iOS Swift rendering.
...
- `services/`: Non-React app logic: SQLite CRUD, geofencing, Supabase sync, photo handling, widget updates.
...
- `plugins/`: Custom Expo config plugins for widget/native build fixes.
...
- `ios/`, `android/`: Native projects are present and matter for widget/auth/plugin work.
...
- `app.config.ts`: Expo config, native package IDs, widgets, permissions, custom plugins, and env-backed native settings.
...
- `utils/supabase.ts`: Supabase client bootstrap and auth/session persistence helpers.
...
- Treat `app.config.ts`, `plugins/`, `ios/`, and `android/` as native-build-sensitive. Change them only when the task is explicitly about native config, widgets, or auth setup.
- `constants/auth.ts` and `.env.local` contain project-specific auth/config values. Do not rotate or replace them casually.
